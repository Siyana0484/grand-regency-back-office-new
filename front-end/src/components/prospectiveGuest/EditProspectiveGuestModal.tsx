...
import { editProspectiveGuest } from "../../apis/ProspectiveGuestApi";
import { useProspectiveGuestStore } from "../../stores/prospectiveGuestStore";

interface EditProspectiveGuestModalProps {
  onClose: () => void;
}

const EditProspectiveGuestModal: React.FC<EditProspectiveGuestModalProps> = ({
  onClose,
}) => {
  const { updateSelectedProspectiveGuest, selectedProspectiveGuest } =
    useProspectiveGuestStore();
  const axiosPrivate = useAxiosPrivate();
  const formik = useFormik({
    initialValues: {
      name: selectedProspectiveGuest?.name ?? "",
      email: selectedProspectiveGuest?.email ?? "",
      phone: selectedProspectiveGuest?.phone ?? "",
      company: selectedProspectiveGuest?.company ?? "",
      description: selectedProspectiveGuest?.description ?? "",
    },
    validationSchema: prospectiveGuestSchema,
    onSubmit: async (values) => {
      const resp = await editProspectiveGuest(
        axiosPrivate,
        values,
      ...
      if (resp.success) {
        updateSelectedProspectiveGuest(resp.updatedProspectiveGuest);
        Success("Prospective Guest Updated successfully");
      }
      onClose();
    },
...
export default EditProspectiveGuestModal;

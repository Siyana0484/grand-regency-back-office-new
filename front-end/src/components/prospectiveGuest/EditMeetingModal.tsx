...
import { deleteMeeting, editMeeting } from "../../apis/meetingApi";
...
const EditMeetingModal: React.FC<EditMeetingModalProps> = ({
  ...
  const formik = useFormik({
    initialValues: {
      date: meetingDetails?.date
        ? new Date(meetingDetails.date).toISOString().split("T")[0]
        : "",
      remarks: meetingDetails?.remarks ?? "",
      attendies: meetingDetails?.attendies ?? [],
    },
    validationSchema: meetingSchema,
    onSubmit: async (values) => {
      const resp = await editMeeting(axiosPrivate, values, meetingDetails?._id);

      if (resp.success) {
        setMeetings((prev) =>
          prev.map((meeting) =>
            meeting._id === resp.updatedMeeting._id
              ? resp.updatedMeeting
              : meeting
          )
        );
  ...
  const handleDeleteMeeting = async (id: string) => {
    ...
    try {
      const resp = await deleteMeeting(axiosPrivate, id);
      if (resp.success) {
        ...
        <form onSubmit={formik.handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-3 mt-4">
            ...
            {hasPermission("meeting:delete") && (
              <button
                type="button"
                onClick={() => handleDeleteMeeting(meetingDetails?._id ?? "")}
                disabled={deleting}
